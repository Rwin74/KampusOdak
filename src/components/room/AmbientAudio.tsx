...
export default function AmbientAudio() {
  ...
  return (
    <div className="relative z-50">
       ...
       <AnimatePresence>
       {isOpen && (
           <motion.div initial={{ opacity: 0, scale: 0.8, x: -20, y: 10 }} animate={{ opacity: 1, scale: 1, x: 0, y: 0 }} exit={{ opacity: 0, scale: 0.8, x: -20, y: 10 }} className="absolute bottom-full left-0 mb-4 flex items-center bg-zinc-900/95 border border-white/20 rounded-full p-2 md:p-3 space-x-1 md:space-x-2 backdrop-blur-3xl shadow-2xl origin-bottom-left">
               {SOUNDS.map(s => {
                  const Icon = s.icon;
       ...
       <button onClick={() => setIsOpen(!isOpen)} className={`p-3 md:p-4 rounded-full text-white shadow-[0_0_30px_rgba(139,92,246,0.3)] border border-white/10 relative transition-all duration-300 ${isOpen ? 'bg-zinc-800' : 'bg-primary/80 backdrop-blur-xl'}`}>
